...
from ui import inject_cursor_glow, inject_styles, render_current_profile, render_feature_importance, render_history, render_prediction, render_system_information
...
with st.container(border=True, key="current-parameters"):
    ...
    st.caption("Set the 11 raw inputs. The saved pipeline applies its own preprocessing.")
...
if predict_clicked:
    try:
        probability, decision = predict(loaded_model,values)
        st.session_state.latest_probability = probability
        st.session_state.history.append({**values,"Risk_Probability":probability*100,"Model decision":"At / above threshold" if decision else "Below threshold"})
    except Exception as error: st.error(f"Prediction failed: {error}")
...
if st.session_state.history:
    ...
    with profile_col: render_current_profile(values,NUMERIC_INPUTS,dark)
